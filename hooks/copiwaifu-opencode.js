...
// version: v3 — tracks opencode's current event surface:
//   - dropped permission.updated (removed upstream; permission.asked +
//     permission.replied now cover the lifecycle)
//   - added session.error (session-level error signal)
// Ships as an opencode V1 plugin ({ id, server }) and POSTs to copiwaifu's
// navigator /event endpoint, mirroring hooks/copiwaifu-hook.js for Claude Code.
...
export default {
  ...
  server: async ({ serverUrl }) => {
    ...
    async function emitThinking(rawSessionID, summary, extra = {}) {
      ...
      await emitOnce(
        ...
        sessionId,
        {
          ...
          summary: truncate(summary) || 'OpenCode is thinking',
          tool_name: 'OpenCode',
    ...
    async function emitPermissionRequest(input) {
      ...
      pendingPermissions.add(rawSessionID)
      await emitOnce(
    ...
    return {
      ...
      event: async ({ event }) => {
        ...
        if (type === 'session.status' && properties.sessionID && properties.status?.type === 'idle') {
          if (pendingPermissions.has(properties.sessionID)) {
            return
          }
        ...
        if (type === 'session.error' && properties.sessionID) {
          const sessionId = getSessionId(properties.sessionID)
          await emit('error', sessionId, {
            working_directory: sessionCwd.get(properties.sessionID),
            session_title: truncate(sessionTitle.get(properties.sessionID)),
            summary: truncate(properties.error || properties.message) || 'OpenCode encountered an error',
            tool_name: 'OpenCode',
            needs_attention: false,
          })
          return
        ...
        if (type === 'session.next.prompted' && properties.sessionID) {
          const text = truncate(properties.prompt?.text || properties.prompt?.message || properties.prompt?.parts?.[0]?.text)
          if (text) {
            await emitThinking(properties.sessionID, text, {
              turn_start: true,
        ...
        if (type === 'session.next.reasoning.started' && properties.sessionID) {
          reasoningText.set(`${properties.sessionID}:${properties.reasoningID}`, '')
          await emitThinking(properties.sessionID, 'OpenCode is thinking')
          return
        }

        if (type === 'session.next.reasoning.delta' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.reasoningID}`
          reasoningText.set(key, `${reasoningText.get(key) || ''}${properties.delta || ''}`)
          return
        }

        if (type === 'session.next.reasoning.ended' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.reasoningID}`
          const text = properties.text || reasoningText.get(key)
          await emitThinking(properties.sessionID, text || 'OpenCode finished thinking')
          reasoningText.delete(key)
          return
        }

        if (type === 'session.next.text.delta' && properties.sessionID) {
          latestAssistantText.set(properties.sessionID, `${latestAssistantText.get(properties.sessionID) || ''}${properties.delta || ''}`)
          return
        }

        if (type === 'session.next.text.ended' && properties.sessionID) {
          if (properties.text) {
            latestAssistantText.set(properties.sessionID, properties.text)
          }
          return
        }

        if (type === 'session.next.tool.called' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.callID}`
          toolInputs.set(key, properties.input)
          toolNames.set(key, properties.tool)
          await emitToolUse(properties.sessionID, properties.callID, properties.tool, properties.input)
          return
        }

        if (type === 'session.next.tool.success' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.callID}`
          await emitToolResult(properties.sessionID, properties.callID, properties.tool || toolNames.get(key), {
            output: properties.content?.map?.(item => item?.text).filter(Boolean).join('\n'),
        ...
        if (type === 'session.next.tool.failed' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.callID}`
          await emitToolResult(properties.sessionID, properties.callID, properties.tool || toolNames.get(key), properties.error, true)
          toolInputs.delete(key)
          toolNames.delete(key)
          return
        }

        if (type === 'session.next.shell.started' && properties.sessionID) {
          toolNames.set(`${properties.sessionID}:${properties.callID}`, 'bash')
          await emitToolUse(properties.sessionID, properties.callID, 'bash', { command: properties.command })
          return
        }

        if (type === 'session.next.shell.ended' && properties.sessionID) {
          const key = `${properties.sessionID}:${properties.callID}`
          await emitToolResult(properties.sessionID, properties.callID, toolNames.get(key) || 'bash', { output: properties.output })
          toolNames.delete(key)
          return
        }
        ...
        if (type === 'question.asked' && properties.sessionID) {
          ...
          pendingPermissions.add(properties.sessionID)
